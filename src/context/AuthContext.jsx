...
export function AuthProvider({ children }) {
  ...
  const signup = async (name, email, password) => {
    const response = await fetch('https://flavr-backend-production.up.railway.app/api/signup', {
      method: 'POST',
  ...
  const login = async (email, password) => {
    const response = await fetch('https://flavr-backend-production.up.railway.app/api/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const uploadProfilePicture = async (file) => {
    ...
    const response = await fetch('https://flavr-backend-production.up.railway.app/api/upload-profile-picture', {
      method: 'POST',
      body: formData
    })
    const data = await response.json()
